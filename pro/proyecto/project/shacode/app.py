...
from helpers import build_file_path, hacerCarpeta, jerarquia, archi2, simpleHash, mode, hayPermiso, hayPermisoFC, addMetadatos, sizeCarpeta
...
@login_required
def shares():
    permisosAlusuario = {"folders": [], "files": []}
    user_id = session["user_id"]

    # Permisos de carpetas compartidas
    permisosEnCarpetas = db.execute(
        "SELECT * FROM permissions WHERE user_id = ? AND folder_id IS NOT NULL",
        user_id
    )
    for permiso in permisosEnCarpetas:
        root_id = permiso["folder_id"]
        todosLosFolders, todosLosFiles = hacerCarpeta(root_id)            # recursión plana
        folderPermiso = archi2(todosLosFolders, todosLosFiles, root_id)             # nueva versión

        # añadir metadatos de quién y nivel de acceso
        addMetadatos(folderPermiso, permisosAlusuario, permiso, "folders")


    # Permisos de archivos sueltos
    permisosEnArchivos = db.execute(
        "SELECT * FROM permissions WHERE user_id = ? AND file_id IS NOT NULL",
        user_id
    )
    for permiso in permisosEnArchivos:
        archivo = db.execute("SELECT * FROM files WHERE id = ?", permiso["file_id"])
        if not archivo:
            continue #se salta si no hay archivo
        archivo = archivo[0]
        addMetadatos(archivo, permisosAlusuario, permiso, "files")



    # parte de los compartidos del usuario
    compartidosDelUsuario = {"folders": [], "files": []}
    # carpetas que ha compartido el usuario
    carpetasCompartidas = db.execute(
        "SELECT * FROM permissions WHERE puser_id = ? AND folder_id IS NOT NULL",
        user_id
    )
    # evitar duplicados -----------------------------------------------------------
    if carpetasCompartidas:
        carpetas = {}
        carpetasSinDuplicar = []
        idCarpetas = []
        for folder in carpetasCompartidas:

            if folder["folder_id"] not in idCarpetas:
                idCarpetas.append(folder["folder_id"])
                carpetasSinDuplicar.append(folder)
            carpetas[folder["folder_id"]] = []
            carpetas[folder["folder_id"]].append(folder["user_id"])


        for permiso in carpetasSinDuplicar:
            root_id = permiso["folder_id"]
            todosLosFolders, todosLosFiles = hacerCarpeta(root_id)            # recursión plana
            folderCompartidos = archi2(todosLosFolders, todosLosFiles, root_id)             # nueva versión

            userIds = []
            permisosUsuarios = {}
            # añadir metadatos de quién y nivel de acceso
            if permiso["folder_id"] in carpetas:
                userIds = carpetas[permiso["folder_id"]]

            for userId in userIds:
                usuarioConPermiso = db.execute("SELECT username FROM users WHERE id = ?", userId)
                if usuarioConPermiso:
                    permisosUsuarios[usuarioConPermiso[0]["username"]] = db.execute("SELECT access_level FROM permissions WHERE user_id = ? AND folder_id = ?", userId, permiso["folder_id"])[0]["access_level"]

            usuarioConPermiso = db.execute("SELECT username FROM users WHERE id = ?", permiso["user_id"])
            if usuarioConPermiso:

                permisosUsuarios[usuarioConPermiso[0]["username"]] = permiso["access_level"]
                folderCompartidos["shared_for"] = permisosUsuarios
                compartidosDelUsuario["folders"].append(folderCompartidos)


     # Permisos de archivos sueltos
     #----------------------------------------------------------------------------------------
    archivosCompartidos = db.execute(
    ...
    if archivosCompartidos:
        ...
        for file in archivosCompartidos:
            if file["file_id"] not in idArchivos:
                idArchivos.append(file["file_id"])
                archivosSinDuplicar.append(file)
            archivos[file["file_id"]] = []
            archivos[file["file_id"]].append(file["user_id"])


        for permiso in archivosSinDuplicar:
            archivo = db.execute("SELECT * FROM files WHERE id = ?", permiso["file_id"])
            if not archivo:
                continue #se salta si no hay archivo
            archivo = archivo[0]
            ...
            permisosUsuarios = {}
            # añadir metadatos de quién y nivel de acceso
            if permiso["file_id"] in archivos:
                userIds = archivos[permiso["file_id"]]

            for userId in userIds:
                usuarioConPermiso = db.execute("SELECT username FROM users WHERE id = ?", userId)
                if usuarioConPermiso:
                    permisosUsuarios[usuarioConPermiso[0]["username"]] = db.execute("SELECT access_level FROM permissions WHERE user_id = ? AND file_id = ?", userId, permiso["file_id"])[0]["access_level"]
            ...
            if usuarioConPermiso:
                ...
                permisosUsuarios[usuarioConPermiso[0]["username"]] = permiso["access_level"]
                archivo["shared_for"] = permisosUsuarios
                compartidosDelUsuario["files"].append(archivo)



    return render_template("shares.html",
        folders = permisosAlusuario["folders"],
        files = permisosAlusuario["files"],
        userFolders = compartidosDelUsuario["folders"],
        userFiles = compartidosDelUsuario["files"]
    )
...
@app.route("/infoArchivo/<int:file_id>")
@login_required
def infoArchivo(file_id):
    try:
        idPropietario = db.execute("SELECT user_id FROM files WHERE id = ?", file_id)
        path = build_file_path(file_id)
        rutaArchivo = os.path.join(app.config["UPLOAD_FOLDER"], str(idPropietario[0]["user_id"]), path)

        datos = os.stat(rutaArchivo)
        size = datos.st_size
        lastModification = datetime.fromtimestamp(datos.st_mtime)
        created = datetime.fromtimestamp(datos.st_ctime)
        name = os.path.basename(rutaArchivo)

        owner = db.execute("SELECT username FROM users WHERE id = ?", idPropietario[0]["user_id"])
        if not owner:
            ownerName = "a stranger"
        else:
            ownerName = owner[0]["username"]
        editors = db.execute("SELECT COUNT(*) AS total FROM permissions WHERE access_level = 'editor' AND file_id = ?", file_id)
        readers = db.execute("SELECT COUNT(*) AS total FROM permissions WHERE access_level = 'reader' AND file_id = ?", file_id)
        return render_template("infoDrive.html",
                               tipo="File",
                               name=name,
                               id=file_id,
                               size=size,
                               ...
                               readers=readers[0]["total"]
                               )
    except FileNotFoundError:
        flash(f"Error: The file '{rutaArchivo}' does not exist.")
        return redirect(url_for("drive"))
    except Exception as e:
        flash(f"Error: {e}")
        return redirect(url_for("drive"))
...
@app.route("/infoCarpeta/<int:folder_id>")
@login_required
def infoCarpeta(folder_id):
    try:
        idPropietario = db.execute("SELECT user_id FROM folders WHERE id = ?", folder_id)
        path = build_folder_path(folder_id)
        rutaCarpeta = os.path.join(app.config["UPLOAD_FOLDER"], str(idPropietario[0]["user_id"]), path)

        datos = os.stat(rutaCarpeta)
        size = sizeCarpeta(rutaCarpeta)
        lastModification = datetime.fromtimestamp(datos.st_mtime)
        created = datetime.fromtimestamp(datos.st_ctime)
        name = os.path.basename(rutaCarpeta)
        ...
        if not owner:
            ...
            ownerName = owner[0]["username"]
        editors = db.execute("SELECT COUNT(*) AS total FROM permissions WHERE access_level = 'editor' AND folder_id = ?", folder_id)
        readers = db.execute("SELECT COUNT(*) AS total FROM permissions WHERE access_level = 'reader' AND folder_id = ?", folder_id)
        return render_template("infoDrive.html",
                               tipo="Folder",
                               name=name,
                               id=folder_id,
        ...
        flash(f"Error: The file '{rutaCarpeta}' does not exist.")
        return redirect(url_for("drive"))
    except Exception as e:
        flash(f"Error: {e}")
        return redirect(url_for("drive"))

# posible refactorizacion para abstraer repeticiones en ambos endpoints
    ...
    if folder_id != "z":
        ...
        if isEntero:
            ...
            if folder:
                ...
                # se puso esta nueva funcion (hay que mejorar la forma en la que se suben archivos)
                # hay que evitar duplicados especialmente en el filename
                idFile = db.execute("SELECT id FROM files WHERE filename = ? AND folder_id = ? AND user_id = ?", archivo.filename, folder_id, user_id)
                # aqui se hizo un cambio en la linea de abajo
                return jsonify({"mensaje": f"The File: '{archivo.filename}' Uploaded successfully (>‿◠)✌ in '{folder[0]["name"]}'!! FileID: '{idFile[0]["id"]}' "}), 200
        ...
        #pronto arreglar este endpoint y todos los relacionados con updates
        # aqui tambien se hizo cambio
        idFile = db.execute("SELECT id FROM files WHERE filename = ? AND folder_id is NULL AND user_id = ?", archivo.filename, user_id)
        return jsonify({"mensaje": f"The File: '{archivo.filename}' Uploaded successfully (>‿◠)✌ in your Root! FileID: '{idFile[0]["id"]}' "}), 200
...
def sha_up():
    ...
    try:
        ...
        return "file does not exist :(", 404
# los ultimos dos endpoints son para bajar scripts que el que
# viene a continuacion despues de ser ejecutado automaticamente se encarga de la configuracion
# instalador de los scripts
@app.route("/download-sha")
def sha():
    path = os.path.join(app.root_path, 'static', "shaCodeDownloader.sh")
