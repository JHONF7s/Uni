...
after(){
	...
	# quitamos -s = silent
        RESPONSE3=$(curl --progress-bar -b .cookies.txt -X POST -H 'Content-Type: multipart/form-data' -F "archivo=@$nameFile" -F "folderid=$folderId " "$API_URL/subir")
        if [ -z "$RESPONSE3" ]; then
...
if [[ $key_ == "Y" || $key_ == "y" ]]; then
    ...
    trap Eliminar SIGINT # si el programa se interrumpe
    file=".tmp.txt"
    if [[ ! -f $file ]]; then
